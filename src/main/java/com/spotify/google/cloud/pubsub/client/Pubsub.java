...
public class Pubsub implements Closeable {
  ...
  private Pubsub(final Builder builder) {
    ...
    final int port = builder.uri.getPort() == -1 ? 443 : builder.uri.getPort();
  ...
  private <T> PubsubFuture<T> request(final String operation, final HttpMethod method, final String path,
    ...
    if (payload != NO_PAYLOAD) {
      ...
      request = new DefaultFullHttpRequest(HTTP_1_1, method, uri, Unpooled.wrappedBuffer(json));
      ...
      request = new DefaultFullHttpRequest(HTTP_1_1, method, uri);
      payloadSize = 0;
    }
    ...
    final CompletableFuture<FullHttpResponse> responseFuture = client.send(request);
    ...
    responseFuture.whenComplete((response, ex) -> {
      ...
      if (status.code() == 404 && method == HttpMethod.GET || method == HttpMethod.DELETE) {
        future.succeed(null);
        return;
      }
      ...
      if (!(status.code() >= 200 && status.code() < 300)) {
        future.fail(new RequestFailedException(status.code(), status.reasonPhrase()));
        return;
      }
      ...
      try {
        future.succeed(Json.read(new ByteBufInputStream(response.content()), responseClass));
      } catch (IOException e) {
        future.fail(e);
      }
    });
